...
export function NotificationsDrawer({
  ...
  const getItemTypeIcon = (type: TaskFiNotificationItem["itemType"]) => {
    switch (type) {
      ...
      case "subtask":
        return <GitBranch className="w-3.5 h-3.5 text-teal-500 shrink-0" />;
      case "bug":
        return <Bug className="w-3.5 h-3.5 text-rose-500 shrink-0" />;
      case "story":
        return (
          <BookmarkSimple className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
        );
      case "pr":
        return (
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
              {filteredNotifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full py-12 text-center text-muted-foreground">
                  <CheckCircle className="w-10 h-10 mb-2.5 text-muted-foreground/30" />
