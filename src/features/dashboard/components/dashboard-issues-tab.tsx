...
export function DashboardIssuesTab({ q }: DashboardIssuesTabProps) {
  ...
  // Filter only bugs
  const bugIssues = allIssues.filter((i) => i.type === "bug");
      ...
      <React.Fragment key={issue.id}>
        ...
        >
          ...
          <TableCell className="align-middle group-hover:border-blue-500 transition-colors">
            <span className="text-[13.5px] font-medium text-slate-900 tracking-tight group-hover:text-blue-700 transition-colors line-clamp-1">
              {issue.summary}
            </span>
          </TableCell>
          <TableCell>
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-[13px] text-slate-500 mt-1">
              {filteredIssues.length} active bugs
            </p>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="bg-white rounded-lg border border-slate-200/60 shadow-sm overflow-hidden flex flex-col w-full">
            <Table>
              ...
              <TableBody>
                {filteredIssues.map((issue) => renderIssueRow(issue))}
              </TableBody>
            </Table>
